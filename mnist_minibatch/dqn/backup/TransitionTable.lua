...
function trans:__init(args)
    self.stateDim = 512*10 --1024 * 25  --51200 --args.stateDim  --modify the state dim from alewrap to cnnweights
    ...
    if (check('/mnt/ramdisk/save/RECENT_S')) then self.recent_s = torch.load('/mnt/ramdisk/save/RECENT_S') else self.recent_s = {} end
    if (check('/mnt/ramdisk/save/RECENT_A')) then self.recent_a = torch.load('/mnt/ramdisk/save/RECENT_A') else self.recent_a = {} end
    if (check('/mnt/ramdisk/save/RECENT_T')) then self.recent_t = torch.load('/mnt/ramdisk/save/RECENT_T') else self.recent_t = {} end
	--TODO read from file
...
function trans:save_recent_x()
	...
	--print (tt)
	torch.save('/mnt/ramdisk/save/RECENT_S', ss)
	torch.save('/mnt/ramdisk/save/RECENT_A', aa)
	torch.save('/mnt/ramdisk/save/RECENT_T', tt)
end
